...
// load session MikroTik
	$editorRole = !empty($_SESSION["mikhmon"]) ? 'admin' : 'manager';
	$session = $_GET['session'];
	if ($editorRole === 'manager' && !empty($_SESSION['manager_session'])) {
		$session = $_SESSION['manager_session'];
	}
	$backUrl = './admin.php?id=sessions';
	if ($editorRole === 'manager') {
		$backUrl = './manager.php?action=dashboard';
	}
...
} elseif ($telplate == "safetmp") {
	$telplatet = "safetmp";
	$popup = "javascript:window.open('./voucher/vpreview.php?usermode=up&template=safetmp&qr=no&session=" . $session . "','_blank','width=380,height=340')";
	$popupQR = "javascript:window.open('./voucher/vpreview.php?usermode=up&template=safetmp&qr=yes&session=" . $session . "','_blank','width=380,height=340')";
} else {
	$telplate = "default";
	$telplatet = "template";
	$popup = "javascript:window.open('./voucher/vpreview.php?usermode=up&qr=no&session=" . $session . "','_blank','width=310,height=310')";
		...
		<div class="row">
			...
			<div class="card-body">
					...
					<table class="table">
						<tr>
							...
							<div class="row">
								...
								<a class="btn" href="<?= $backUrl; ?>" style="background:#34495e;color:#fff;"><i class="fa fa-arrow-left"></i></a>
								...
								<div class="input-group-3">
	    							<select style="padding:4.2px;"  class="group-item group-item-m" onchange="window.location.href=this.value+'&session=<?= $session; ?>';">
	    								...
	    								<option value="./admin.php?id=editor&template=safetmp">SafeTmp</option>
	    							</select>
	    						</div>
	        	...
	        	<textarea class="bg-dark" id="editorMikhmon" name="editor" style="width:100%" height="700">
						...
						echo file_get_contents('./voucher/safetmp.php');
