...
include_once('./include/managers_config.php');
...
if ($manager_index_logged_in) {
  if (isset($_GET['hotspot-user']) && $_GET['hotspot-user'] !== '' && $_GET['hotspot-user'] !== 'generate') {
    ob_end_clean();
    header('Location: ./manager.php?action=dashboard');
    exit;
  }

  $manager_denied_params = array(
    ...
    'reset-hotspot-user', 'remove-hotspot-user-by-comment',
    'remove-hotspot-user-expired', 'enable-hotspot-user',
  ...
  if (!$manager_allowed_index_route) {
    ob_end_clean();
    header('Location: ./manager.php?action=dashboard');
    exit;
  }

  $_GET['session'] = isset($_SESSION['manager_session']) ? $_SESSION['manager_session'] : '';
}
...
if (!isset($_SESSION["mikhmon"]) && !$manager_index_logged_in) {
...
if ($livereport == "enable" || $livereport == "") {
  if($_SESSION[$session.'sdate'] != $_SESSION[$session.'idhr']){
    ...
    echo '$("#r_sellers").load("./report/livereport.php?session=' . $session . ' #r_sellers");';
  ...
  echo  '
    ...
    $("#r_sellers").load("./report/livereport.php?session=' . $session . ' #r_sellers"); 
  }, interval2);
 ';}
  echo ' 
  function cancelPage(){
